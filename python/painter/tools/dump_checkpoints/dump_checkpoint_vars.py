...
def get_checkpoint_dumper(model_type, checkpoint_file, output_dir, remove_variables_regex):
  ...
  if model_type == 'tensorflow':
    from server.tools.dump_checkpoints.tensorflow_checkpoint_dumper import TensorflowCheckpointDumper

    return TensorflowCheckpointDumper(
      checkpoint_file, output_dir, remove_variables_regex)
  elif model_type == 'pytorch':
    from server.tools.dump_checkpoints.pytorch_checkpoint_dumper import PytorchCheckpointDumper

    return PytorchCheckpointDumper(
      checkpoint_file, output_dir, remove_variables_regex)
  else:
    raise Exception('Currently, "%s" models are not supported'.format(model_type))
