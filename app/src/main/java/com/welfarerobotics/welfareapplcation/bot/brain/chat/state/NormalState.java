...
public class NormalState implements ChatState {
    ...
    @Override public ChatState think(String intent, String speech, Activity activity) throws IOException {
        ...
        else Brain.hippocampus.rememberIntent(intent);
        ...
        if (intent.equals("날짜") || intent.equals("국가") || intent.equals("지역") ||
            ...
            return CONTEXT_STATE.think(intent, speech, activity); // 문맥으로 전환
        ...
        if (intent.equals("볼륨다운")) return VolumeUpScenario.process(speech);
        if (intent.equals("잘못들음")) return PardonScenario.process(speech);
        if (intent.equals("동화")) return FairytaleScenario.process(speech, activity);
        if (intent.equals("와이파이")) return WifiScenario.process(speech, activity);
