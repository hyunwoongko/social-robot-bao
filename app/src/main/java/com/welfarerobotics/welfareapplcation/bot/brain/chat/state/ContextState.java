...
public class ContextState implements ChatState {
    ...
    @Override public ChatState think(String intent, String speech) throws IOException {
        ...
        if (intent.equals("지역")) { // 전주는? || 부산은? || 대구는 어때?
        ...
        if (intent.equals("국가")) { // 미국은? || 일본어는? || 독일어는?
            if (prevIntent.equals("번역"))
                return TranslateScenario.process(Brain.hippocampus.getThoughtSentence(), Oblivion::forgetLang);
        }

        // 날씨 <-> 먼지 간의 의도 변환
        if (intent.equals("날씨") && prevIntent.equals("먼지")) return WeatherScenario.process(speech);
        if (intent.equals("먼지") && prevIntent.equals("날씨")) return DustScenario.process(speech);
        return this;
    }
